import datetime
from backend import pandas_utils, data_utils, docx_utils
from backend.classes.common_config import CommonConfig
from typing import Any


def _make_date_string_representation(
        date: datetime.date
) -> tuple[str, str, str]:
    month_names_dict = {
        1: 'января',
...
def runtime(
    ...
    try:
        ...
        data_summarized = data_utils.get_students_stats_with_discipline_configs_grouped_by_category_summarized(
    ...
    try:
        data_ready = data_utils.get_students_configs(
            data_summarized,
            pandas_utils.read_xlsx(state_holder['diploma_file_path']))
    except Exception:
        return 2
