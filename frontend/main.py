...
RUNTIME_RET_CODES = {
    ...
    3: 'Некорректный формат файла шаблона выписки или перезаписываемый файл открыт!',
    ...
    if len(file_paths) != 0:
        if not all(os.path.exists(file_path) for file_path in file_paths):
            messagebox.showerror(
                title='Ошибка', message='Одного или нескольких выбранных файлов не существует!'
            )
            return
...
def push_semester_files_button_up(
    ...
    semester_files_listbox.insert(selected_index - 1, selected_file_path)
    semester_files_listbox.selection_set(selected_index - 1)

    (
        state_holder['semester_files_paths'][selected_index],
        state_holder['semester_files_paths'][selected_index - 1],
    ) = (
        state_holder['semester_files_paths'][selected_index - 1],
        state_holder['semester_files_paths'][selected_index],
    )
...
def push_diplomas_file_button(
    ...
    if file_path != '':
        if not os.path.exists(file_path):
            messagebox.showerror(title='Ошибка', message='Выбранного файла не существует!')
            return

        state_holder['diploma_file_path'] = file_path
        diploma_files_text.config(state=tk.NORMAL)
        diploma_files_text.delete('1.0', tk.END)
        diploma_files_text.insert('1.0', state_holder['diploma_file_path'])
...
def push_template_file_button(
    ...
    if file_path != '':
        ...
        template_file_text.insert('1.0', state_holder['template_file_path'])
...
def push_save_directory_button(
    ...
    global state_holder
    directory_path = filedialog.askdirectory(title='Выберите директорию сохранения выписок')
    if directory_path != '':
        if not os.path.exists(directory_path):
            messagebox.showerror(title='Ошибка', message='Выбранной директории не существует!')
            return
        ...
        save_directory_text.insert('1.0', state_holder['save_directory_path'])
...
def push_make_statements_button() -> None:
    ...
    if any(state is None for state in state_holder.values()):
        messagebox.showerror(title='Ошибка', message='Одно или несколько полей не заполнены!')
        return
    ...
    if ret_code in [1, 2, 3]:
        messagebox.showerror(title='Ошибка', message=RUNTIME_RET_CODES[ret_code])
    else:
        messagebox.showinfo(title='Успех', message=RUNTIME_RET_CODES[ret_code])
...
def main() -> None:
    main_window = tk.Tk()
    main_window.title(
        "Программное средство для формирования выписки из зачётно-экзаменационной ведомости"
    )
    ...
    canvas.bind_all("<MouseWheel>", lambda event: _on_mouse_wheel(event, canvas=canvas))
    ...
    app_name_label = tk.Label(
        root, text="Формирование выписок из зачётно-экзаменационной ведомости", font=("Arial", 18)
    )
    app_name_label.grid(column=0, row=0, columnspan=4, padx=5, pady=5, sticky=tk.EW)

    get_new_separator(root).grid(column=0, row=1, columnspan=4, sticky=tk.EW)

    semester_count_label = tk.Label(
        root, text="Выберите количество зачётно-экзаменационных ведомостей:", font=("Arial", 10)
    )
    semester_count_label.grid(column=0, row=2, columnspan=2, padx=5, pady=5, sticky=tk.W)

    global SEMESTER_COUNT_COMBOBOX_VALUES
    semester_count_combobox = ttk.Combobox(
        root, values=SEMESTER_COUNT_COMBOBOX_VALUES, state="readonly"
    )
    ...
    semester_count_combobox.grid(column=2, row=2, columnspan=2, padx=5, pady=5, sticky=tk.EW)

    get_new_separator(root).grid(column=0, row=3, columnspan=4, sticky=tk.EW)
    ...
    semester_files_label.grid(column=0, row=4, columnspan=2, padx=5, pady=5, sticky=tk.W)
    ...
    semester_files_button = tk.Button(
        ...
        command=lambda: push_semester_files_button(
            semester_files_listbox, semester_files_button_up, semester_files_button_down
        ),
    )
    semester_files_button.grid(column=2, row=4, columnspan=2, padx=5, pady=5, sticky=tk.EW)

    semester_files_listbox = tk.Listbox(
        root, height=10, width=50, font=("Arial", 10), selectmode=tk.SINGLE, activestyle=tk.NONE
    )
    semester_files_listbox.insert(tk.END, 'Файлы зачётно-экзаменационных ведомостей не выбраны...')
    semester_files_listbox.config(state=tk.DISABLED)
    semester_files_listbox.grid(
        column=0, row=5, padx=5, pady=5, columnspan=3, rowspan=2, sticky=tk.EW
    )
    ...
    semester_files_button_up.grid(column=3, row=5, padx=5, pady=5, sticky=tk.EW)
    ...
    semester_files_button_down.grid(column=3, row=6, padx=5, pady=5, sticky=tk.EW)

    diplomas_file_label = tk.Label(
        root, text="Выберите файл с темами дипломных проектов:", font=("Arial", 10)
    )

    diplomas_file_label.grid(column=0, row=7, columnspan=2, padx=5, pady=5, sticky=tk.W)
    ...
    diplomas_file_button.grid(column=2, row=7, columnspan=2, padx=5, pady=5, sticky=tk.EW)
    ...
    diplomas_file_text.insert('1.0', 'Файл с темами дипломных проектов не выбран...')
    diplomas_file_text.config(state=tk.DISABLED)
    diplomas_file_text.grid(column=0, row=8, padx=5, pady=5, columnspan=4, sticky=tk.EW)

    get_new_separator(root).grid(column=0, row=9, columnspan=4, sticky=tk.EW)

    template_file_label = tk.Label(root, text="Выберите файл шаблона выписки:", font=("Arial", 10))
    template_file_label.grid(column=0, row=10, columnspan=2, padx=5, pady=5, sticky=tk.W)
    ...
    template_file_button.grid(column=2, row=10, columnspan=2, padx=5, pady=5, sticky=tk.EW)
    ...
    template_file_text.insert('1.0', 'Файл шаблона выписки не выбран...')
    template_file_text.config(state=tk.DISABLED)
    template_file_text.grid(column=0, row=11, padx=5, pady=5, columnspan=4, sticky=tk.EW)

    get_new_separator(root).grid(column=0, row=12, columnspan=4, sticky=tk.EW)

    save_directory_label = tk.Label(
        root, text="Выберите директорию сохранения выписок:", font=("Arial", 10)
    )
    save_directory_label.grid(column=0, row=13, columnspan=2, padx=5, pady=5, sticky=tk.W)
    ...
    save_directory_button.grid(column=2, row=13, columnspan=2, padx=5, pady=5, sticky=tk.EW)
    ...
    save_directory_text.insert('1.0', 'Директория сохранения выписок не выбрана...')
    save_directory_text.config(state=tk.DISABLED)
    save_directory_text.grid(column=0, row=14, padx=5, pady=5, columnspan=4, sticky=tk.EW)

    get_new_separator(root).grid(column=0, row=15, columnspan=4, sticky=tk.EW)
    ...
    speciality_name_label.grid(column=0, row=16, columnspan=4, padx=5, pady=5, sticky=tk.EW)
    ...
    speciality_name_entry.bind(
        '<KeyRelease>', lambda event: on_key_release_entry('speciality_name', speciality_name_entry)
    )
    speciality_name_entry.grid(column=0, row=17, columnspan=4, padx=5, pady=5, sticky=tk.EW)
    ...
    speciality_code_label.grid(column=0, row=18, columnspan=4, padx=5, pady=5, sticky=tk.EW)
    ...
    speciality_code_entry.bind(
        '<KeyRelease>', lambda event: on_key_release_entry('speciality_code', speciality_code_entry)
    )
    speciality_code_entry.grid(column=0, row=19, columnspan=4, padx=5, pady=5, sticky=tk.EW)

    get_new_separator(root).grid(column=0, row=20, columnspan=4, sticky=tk.EW)
    ...
    speciality_area_name_label.grid(column=0, row=21, columnspan=4, padx=5, pady=5, sticky=tk.EW)
    ...
    speciality_area_name_entry.grid(column=0, row=22, columnspan=4, padx=5, pady=5, sticky=tk.EW)
    ...
    speciality_area_code_label.grid(column=0, row=23, columnspan=4, padx=5, pady=5, sticky=tk.EW)
    ...
    speciality_area_code_entry.grid(column=0, row=24, columnspan=4, padx=5, pady=5, sticky=tk.EW)

    get_new_separator(root).grid(column=0, row=25, columnspan=4, sticky=tk.EW)
    ...
    start_date_label.grid(column=0, row=26, columnspan=4, padx=5, pady=5, sticky=tk.EW)

    start_date_dateentry = DateEntry(root, date_pattern='DD-MM-YYYY', locale='ru', state='readonly')
    start_date_dateentry.bind(
        '<<DateEntrySelected>>',
        lambda event: on_date_entry_selected('start_date', start_date_dateentry),
    )
    start_date_dateentry.grid(column=0, row=27, columnspan=4, padx=5, pady=5, sticky=tk.EW)
    ...
    end_date_label.grid(column=0, row=28, columnspan=4, padx=5, pady=5, sticky=tk.EW)

    end_date_dateentry = DateEntry(root, date_pattern='DD-MM-YYYY', locale='ru', state='readonly')
    end_date_dateentry.bind(
        '<<DateEntrySelected>>',
        lambda event: on_date_entry_selected('end_date', end_date_dateentry),
    )
    end_date_dateentry.grid(column=0, row=29, columnspan=4, padx=5, pady=5, sticky=tk.EW)

    get_new_separator(root).grid(column=0, row=30, columnspan=4, sticky=tk.EW)
    ...
    statement_date_label.grid(column=0, row=31, columnspan=4, padx=5, pady=5, sticky=tk.EW)

    statement_date_dateentry = DateEntry(
        root, date_pattern='DD-MM-YYYY', locale='ru', state='readonly'
    )
    statement_date_dateentry.bind(
        '<<DateEntrySelected>>',
        lambda event: on_date_entry_selected('statement_date', statement_date_dateentry),
    )
    statement_date_dateentry.grid(column=0, row=32, columnspan=4, padx=5, pady=5, sticky=tk.EW)

    get_new_separator(root).grid(column=0, row=33, columnspan=4, sticky=tk.EW)
    ...
    make_statements_button.grid(column=0, row=34, columnspan=4, padx=5, pady=5, sticky=tk.EW)
    root.mainloop()
