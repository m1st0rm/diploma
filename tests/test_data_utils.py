...
def sample_diploma_themes_df():
    return DataFrame({
        ...
        "Тема димпломного проекта": ["Тема 1", "Тема 2"]
    })
...
def test_get_students_stats_with_discipline_configs_grouped_by_category_summarized(sample_students_stats_raw):
    students_stats = get_students_stats_with_discipline_configs(sample_students_stats_raw)
    grouped_by_category = get_students_stats_with_discipline_configs_grouped_by_category(students_stats)
    result = get_students_stats_with_discipline_configs_grouped_by_category_summarized(grouped_by_category)
...
def test_get_students_configs(sample_students_stats_raw, sample_diploma_themes_df):
    students_stats = get_students_stats_with_discipline_configs(sample_students_stats_raw)
    grouped_by_category = get_students_stats_with_discipline_configs_grouped_by_category(students_stats)
    summarized_by_category = get_students_stats_with_discipline_configs_grouped_by_category_summarized(grouped_by_category)
